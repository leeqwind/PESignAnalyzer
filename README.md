...
This program is used to get signature information from PE files which signed by a/some embedded code signature certificate(s). Supporting multi-signed &amp; cert-chain. Runned on Windows 7+ OS.
...
这个程序用来从由1个或多个嵌入式代码签名证书所签名的PE文件中获取签名信息。支持多签名；支持证书链的提取。运行在Windows7及以上的操作系统平台。
...
If you transfer the path of a multi-signatured file to PESignAnalyzer binary file, it will show information as shown below. Every `[The X Sign Info]` means a chunk of completed information of a signature block.
