...
typedef struct _SIGN_NODE_INFO {
	...
	list<CERT_NODE_INFO> CertNodeChain;
...
BOOL CertificateCheck(const WCHAR *szCurrFullPath)
{
	...
	ret = CheckFileDigitalSignature(imagePath.c_str(), TRUE,
		NULL,
		cataFile,
		signType,
		signChain);
	...
	if (ret)
	{
		...
		unsigned int idx = 0;
		list<SIGN_NODE_INFO>::iterator iter;
		for (iter = signChain.begin();
			iter != signChain.end(); iter++)
		{
			...
			list<CERT_NODE_INFO>::iterator iter1;
...
BOOL GetNestedSignerInfo(PCMSG_SIGNER_INFO pSignerInfo, DWORD cbSignerSize,
	list<SIGNDATA_HANDLE> & NestedChain)
{
	BOOL			fReturn      = FALSE;
	HCRYPTMSG		hNestedMsg   = NULL;
	DWORD			n            = 0x00;
	BYTE			*pbCurrData  = NULL;
	BYTE			*pbNextData  = NULL;
	...
	std::wstring & CRLpoint)
{
	...
	if (!(pe = CertFindExtension(szOID_CRL_DIST_POINTS,
		cExtensions,
		rgExtensions)))
	{
		return FALSE;
...
BOOL GetSignerCertificateInfo(LPCWSTR filename, list<SIGN_NODE_INFO> & signChain)
{
	...
	list<SIGNDATA_HANDLE>	SignDataChain;
	...
	if (!MyCryptMsgGetParam(hAuthCryptMsg,
		...
		iter != SignDataChain.end(); iter++)
	{
		...
		while (pCurrContext)
		{
			...
			bReturn = CalculateCertAlgorithm(szObjId,
				CertNodeInfo.signAlgorithm);
